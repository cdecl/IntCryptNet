...
{
    /// <summary>
    /// 시드(SEED) 알고리즘 Ver 1.0
    /// </summary>
    /// <remark>06.06.21 - ProtoType - 서아름 (gamm4266@naver.com)</newpara>
    /// <remark>06.06.21 - 커스터마이징 - 방실</newpara>
    internal class SeedAlgorithm
    {
        #region SSN

        private uint[] SS0 = new uint[256]
	    {
        ...
        private byte GetB0(uint A)
        {
            return BitConverter.GetBytes(A)[0];
        }
        private byte GetB1(uint A)
        {
            return BitConverter.GetBytes(A)[1];
        }
        private byte GetB2(uint A)
        {
            return BitConverter.GetBytes(A)[2];
        }
        private byte GetB3(uint A)
        {
            return BitConverter.GetBytes(A)[3];
        }

        private void EncRoundKeyUpdate0(ref uint[] K, int idx, ref uint A, ref uint B, ref uint C, ref uint D, uint KC)
        {
            uint T0;
            uint T1;
            T0 = A;
        ...
        private void EncRoundKeyUpdate1(ref uint[] K, int idx, ref uint A, ref uint B, ref uint C, ref uint D, uint KC)
        {
            uint T0;
            uint T1;
            T0 = C;
        ...
        private uint EndianChange(uint dwS)
        {
        ...
        private void SeedRound(ref uint L0, ref uint L1, uint R0, uint R1, uint[] K, int idx)
        {
            uint T0, T1;
            T0 = R0 ^ K[0 + idx];
            T1 = R1 ^ K[1 + idx];
        ...
        private void XorByte(ref byte[] operand1, byte[] operand2)
        {
        ...
        internal byte[] Encrypt(byte[] Data, byte[] seedKey)
        {
            return Encrypt(Data, seedKey, false);
        }

        internal byte[] Encrypt(byte[] Data, byte[] seedKey, bool cbcPad)
        {
        ...
        internal byte[] Decrypt(byte[] Data, byte[] seedKey)
        {
            return Decrypt(Data, seedKey, false);
        }

        internal byte[] Decrypt(byte[] Data, byte[] seedKey, bool cbcPad)
        {
